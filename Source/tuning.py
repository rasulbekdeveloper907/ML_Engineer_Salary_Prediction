...
class ModelTuner:
    """
    Ushbu klass Grid Search va Randomized Search yordamida
    model tuning uchun ishlatiladi.
    """

    def __init__(self, model, param_grid, cv=5, scoring="r2", n_iter=10, random_state=42):
        """
        Args:
            model: sklearn yoki xgboost model obyekt
            param_grid: lug'at (model parametrlari uchun)
            cv: cross-validation bo‘lak soni
            scoring: model baholash mezoni
            n_iter: RandomizedSearch uchun iteration soni
            random_state: natijalarni takrorlash uchun
        """
        self.model = model
    ...
    def grid_search(self, X, y):
        """
        GridSearchCV yordamida model tuning.
        """
    ...
    def randomized_search(self, X, y):
        """
        RandomizedSearchCV yordamida model tuning.
        """
